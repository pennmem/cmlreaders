...
class EEGContainer(object):
    ...
    def __init__(
        ...
        if epochs is not None:
            if len(epochs) != self.data.shape[0]:
                raise ValueError(
                    "epochs must be the same length as the first " "data dimension"
                )
            self.epochs = epochs
        else:
        ...
        if channels is not None:
            ...
            self.channels = np.linspace(
                1, self.data.shape[1], self.data.shape[1], dtype=int
            ).tolist()

        self.attrs = attrs if attrs is not None else {}

    def _make_time_array(self, tstart):
        rate = 1000.0 / self.samplerate
        n_samples = self.data.shape[-1]
        return np.arange(tstart, n_samples * rate + tstart, rate)

    @classmethod
    def concatenate(
        cls, containers: List["EEGContainer"], dim="events"
    ) -> "EEGContainer":
        ...
        def check_samples():
            if not all([s.shape[-1] == containers[0].shape[-1] for s in containers]):
                raise ValueError(
                    "Number of samples must match to concatenate" " events"
                )
        ...
        def check_channels():
            if not all(
                [np.all(s.channels == containers[0].channels) for s in containers]
            ):
        ...
        def check_starts():
            ...
            step = containers[0].samplerate / 1000.0
            last = containers[0].time[-1]
            for s in containers[1:]:
                if last + step != s.time[0]:
                    raise ValueError(
                        "Start times are not properly aligned for" " concatenation"
                    )
                last += step
    ...
    def resample(self, rate: Union[int, float]) -> "EEGContainer":
        ...
        new_data, _ = scipy.signal.resample(self.data, new_len, t=self.time, axis=-1)
        return EEGContainer(
            new_data,
            rate,
            epochs=self.epochs,
    ...
    def to_ptsa(self, recarray=False) -> "TimeSeries":  # noqa: F821
        ...
        recarray
            ...
            (use for session with 'stim_params' field with dict entries).
    ...
    def to_mne(self) -> Union["mne.EpochsArray", "mne.io.RawArray"]:  # noqa: F821
        ...
        if "mne_info" not in self.attrs:
            info = mne.create_info(
                [str(c) for c in self.channels], self.samplerate, ch_types="eeg"
            )
        ...
        # Return EpochsArray if loading epoched data
            ...
            eeg = mne.EpochsArray(self.data, info, tmin=self.time[0] / 1000.0)
